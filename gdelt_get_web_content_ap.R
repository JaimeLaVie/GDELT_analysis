  ...
  if (str_detect(gdelt_files[num], ".csv$") == TRUE){
    ...
    for (i in 1:length_csv_file){
      ...
      if (is.na(url) == FALSE){  #确保url不为空
        if (str_detect(url,'/apnews.com') == TRUE){ # && str_detect(url,'/live/') == FALSE && str_detect(url,'mediacentre/') == FALSE && str_detect(url,'/sport/') == FALSE && str_detect(url,'/future/') == FALSE && str_detect(url,'/culture/') == FALSE && str_detect(url,'/programmes/') == FALSE && str_detect(url,'/cbbc/') == FALSE){
